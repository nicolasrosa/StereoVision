...
void setStereoSGBMparams(StereoSGBM* sgbm,int numRows,int numChannels,bool showStereoSGBMparams){
	int trackbarsAux[10];
...
//void presetStereoSGBMparams(StereoSGBM* sgbm,int width){
//	//PreSetup StereoSGBM Parameters
//	numberOfDisparities = numberOfDisparities > 0 ? numberOfDisparities : ((int)(width/8) + 15)/4 & -16;
//
//	sgbm->setPreFilterCap(31);
//	sgbm->setBlockSize(SADWindowSize > 0 ? SADWindowSize : 9);
//	sgbm->setMinDisparity(0);
//	sgbm->setNumDisparities(numberOfDisparities);
//	//sgbm->setTextureThreshold(10);
...
int main(int, char**){
	...
	bool showInputImage=true,showXYZ=false,showStereoSGBMparams=false,showFPS=false,showDisparityMap=false,show3Dreconstruction=false,showDiffImage=false;
	...
	while(key!='q'){
		...
		setStereoSGBMparams(sgbm,imageL[0].rows,imageL[0].channels(),showStereoSGBMparams);
		...
		sgbm->compute(imageL[0],imageR[0],disp);
		...
		if(key=='5')
			showFPS = !showFPS;
		if(key=='6')
			showStereoSGBMparams = !showStereoSGBMparams;
		if(key=='7')
